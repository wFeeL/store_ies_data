...
def get_forecast_bundle(psm: Any) -> Dict[str, Dict[str, Any]]:
    # Prefer explicit env override, else real psm.forecasts if available, else external CSV fallback.
    env_path = os.environ.get('IES_FORECAST_FILE')
    if env_path:
    ...
    for row in object_rows:
        ...
        if typ == 'solar':
            ...
            if fc_now > 0.05 and actual >= 0.0:
                ...
                model['bias'] = 0.94 * safe_float(model.get('bias', base_bias), base_bias) + 0.06 * clamp(est_bias, 0.55, 1.20)
            else:
...
def build_forecast_profile(bundle: Dict[str, Dict[str, Any]], object_rows: List[Dict[str, Any]], game_length: int) -> Dict[str, Any]:
    ...
    for t in range(rows):
        ...
        combo_score = 0.50 * z_sun + 0.35 * z_load + 0.15 * z_wind
        risk_score = 0.55 * max(0.0, -z_sun) + 0.25 * max(0.0, -z_wind) + 0.35 * max(0.0, z_load)
        ticks.append({
            'tick': t,
            'sun': sun[t],
            'wind': wind[t],
...
def compute_target_soc(cfg: Dict[str, Any], total_capacity: float, total_soc: float, future: List[Dict[str, Any]], fill_ratio: float, tick: int, game_length: int, loss_ratio: float = 0.10, profile_ctx: Optional[Dict[str, Any]] = None) -> float:
    ...
    if profile_ctx:
        ...
        if current.get('mixed_peak'):
            target += 0.06 * total_capacity
        if next_risk_in is not None and 0 <= next_risk_in <= 14:
            target += (0.04 + 0.08 * (1.0 - min(next_risk_in, 14) / 14.0)) * total_capacity
        if avg_risk > avg_combo + 0.18:
...
def storage_policy(cfg: Dict[str, Any], storages: List[Dict[str, Any]], balance_now: float, useful_now: float, future: List[Dict[str, Any]], fill_ratio: float, tick: int, game_length: int, loss_ratio: float = 0.10, profile_ctx: Optional[Dict[str, Any]] = None) -> Tuple[List[Tuple[str, float]], List[Tuple[str, float]], Dict[str, Any]]:
    ...
    if next_risk_in is not None and 0 <= next_risk_in <= 10:
        floor_frac = max(floor_frac, 0.12)
    floor_soc = total_capacity * floor_frac
    protected_soc = max(floor_soc, 0.40 * target_soc if chronic_deficit else 0.65 * target_soc)
    if current_profile.get('mixed_peak') or current_profile.get('solar_active'):
        protected_soc = max(protected_soc, 0.72 * target_soc)
    charge_total = 0.0
    discharge_total = 0.0
    discharge_for_market = 0.0
    ...
    if current_deficit > 0.0 and discharge_budget > 0.0:
        ...
        if chronic_deficit or current_profile.get('protect_bias', 0.0) > 0.5:
            desired += 0.10 * deficit_sum + 0.20 * next_deficit
    ...
    if current_surplus > 0.0 and charge_room > 0.0:
        ...
        if chronic_deficit or total_soc < target_soc or next_deficit > 1.0:
            desired_charge = max(desired_charge, current_surplus * 1.05)
        if current_profile.get('mixed_peak') or current_profile.get('charge_bias', 0.0) > 0.5:
            desired_charge = max(desired_charge, current_surplus * 1.18)
        if next_risk_in is not None and 0 <= next_risk_in <= 16:
            desired_charge = max(desired_charge, current_surplus * 1.28)
        charge_total = min(total_charge_rate, charge_room, desired_charge)
    elif current_surplus <= 0.0 and next_surplus > 2.5 and total_soc < target_soc and charge_room > 0.0:
...
def compute_reserve(state: Dict[str, Any], future: List[Dict[str, Any]], object_rows: List[Dict[str, Any]], network_losses: float, useful_supply_now: float, profile_ctx: Optional[Dict[str, Any]] = None) -> float:
    ...
    reserve = 0.18 + 0.32 * abs_err + 0.08 * max(0.0, loss_ratio - 0.10) * max(gen_total, useful_supply_now)
    reserve += 0.07 * wind_share * max(wind_gen, next_useful)
    if next_useful < useful_supply_now:
        reserve += 0.16 * (useful_supply_now - next_useful)
    if next_balance < -2.0:
        reserve += 0.05 * abs(next_balance)

    if profile_ctx:
        current = profile_ctx.get('current', {})
        avg_combo = safe_float(profile_ctx.get('avg_combo_6', 0.0), 0.0)
        avg_risk = safe_float(profile_ctx.get('avg_risk_6', 0.0), 0.0)
        next_risk_in = profile_ctx.get('next_risk_in')
        if current.get('mixed_peak') and current.get('solar_peak'):
            reserve *= 0.84
        elif current.get('solar_active') and current.get('load_peak'):
            reserve *= 0.90
        if current.get('wind_peak') and wind_share > 0.30:
            reserve *= 1.08
        if avg_risk > avg_combo + 0.18:
            reserve *= 1.10
        if next_risk_in is not None and 0 <= next_risk_in <= 5:
            reserve *= 1.12
        if current.get('tail_low_load'):
            reserve *= 0.88

    if useful_supply_now > 0.0:
        reserve = min(reserve, max(MIN_RESERVE, 0.20 * useful_supply_now))
...
def build_ladder(sell_volume: float, market_ref: float, fill_ratio: float, max_tickets: int, cfg: Dict[str, Any], buy_ref: Optional[float] = None, profile_ctx: Optional[Dict[str, Any]] = None) -> List[Tuple[float, float]]:
    ...
    avg_combo = safe_float(profile_ctx.get('avg_combo_6', 0.0), 0.0) if profile_ctx else 0.0
    avg_risk = safe_float(profile_ctx.get('avg_risk_6', 0.0), 0.0) if profile_ctx else 0.0
    if current.get('mixed_peak'):
        base_ref += 2.0 * step
    elif current.get('solar_active') and current.get('load_peak'):
        base_ref += 1.0 * step
    elif avg_risk > avg_combo + 0.18:
        base_ref -= 1.0 * step
    base_ref = clamp(base_ref, gp_price + step, market_cap - step)

    if sell_volume < 1.5:
        prices = [gp_price, min(base_ref, gp_price + 4 * step)]
        shares = [0.75, 0.25]
    elif fill_ratio < 0.65:
        prices = [gp_price, gp_price + 2 * step, min(base_ref, gp_price + 6 * step)]
        shares = [0.72, 0.20, 0.08]
    elif current.get('mixed_peak') and sell_volume >= 2.0:
        prices = [gp_price + step, min(base_ref, gp_price + 5 * step), min(base_ref + 4 * step, market_cap - step)]
        shares = [0.54, 0.28, 0.18]
    elif fill_ratio > 0.92:
        prices = [gp_price + step, min(base_ref, gp_price + 5 * step), min(base_ref + 4 * step, market_cap - step)]
        shares = [0.58, 0.27, 0.15]
    else:
        prices = [gp_price + step, min(base_ref, gp_price + 4 * step), min(base_ref + 3 * step, market_cap - step)]
        shares = [0.66, 0.24, 0.10]
