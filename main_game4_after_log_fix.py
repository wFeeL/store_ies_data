...
def get_forecast_bundle(psm: Any) -> Dict[str, Dict[str, Any]]:
    # Prefer real psm.forecasts if they are populated for the active game.
    # Use IES_FORECAST_FILE only as an explicit fallback.
    ...
    if isinstance(raw, list):
        for idx, item in enumerate(raw):
            ...
            data = list(item.get('data', [])) if isinstance(item, dict) else list(item)
            out[name] = {
                'data': data,
                'spread': safe_float(item.get('spread', 0.0), 0.0) if isinstance(item, dict) else 0.0,
                'source': 'psm',
            }
    else:
        for name in FORECAST_SERIES_ORDER:
...
def get_model(state: Dict[str, Any], key: str, kind: str) -> Dict[str, Any]:
    ...
    if mkey not in models:
        if kind == 'solar':
            models[mkey] = {
                'kind': kind,
                'factor': SOLAR_SEED_FACTORS.get(key, 0.78),
                'err': 0.8,
            }
        elif kind == 'wind':
            models[mkey] = {
                'kind': kind,
                'factor': WIND_SEED_FACTORS.get(key, 0.0058),
                'rot_factor': 92.0,
                'wind_to_rot': 0.040,
                'rot_curve': {},
                'max_power_seen': 0.0,
            ...
            models[mkey] = {'kind': kind, 'bias': LOAD_BIAS_PRIOR, 'err': 0.6}
    return models[mkey]
...
def get_type_load_prior(obj_type: str) -> float:
    return safe_float(LOAD_TYPE_PRIOR.get(obj_type, LOAD_BIAS_PRIOR), LOAD_BIAS_PRIOR)


def blended_load_bias(state: Dict[str, Any], obj_type: str, object_bias: Optional[float] = None) -> float:
    type_prior = get_type_load_prior(obj_type)
    total_bias = safe_float(state.get('load_bias_total', LOAD_BIAS_PRIOR), LOAD_BIAS_PRIOR)
    base = 0.58 * type_prior + 0.42 * total_bias
    if object_bias is None:
        return base
    return 0.45 * safe_float(object_bias, base) + 0.55 * base
...
def update_models(state: Dict[str, Any], object_rows: List[Dict[str, Any]], weather: Dict[str, float], bundle: Dict[str, Dict[str, Any]], tick: int, cfg: Optional[Dict[str, Any]] = None, total_consumed: Optional[float] = None, total_losses: Optional[float] = None) -> None:
    ...
    if total_consumed is not None and total_fc_now > 1e-6:
        ...
        load_bias = clamp(load_bias, 0.68, 1.10)
    ...
    for row in object_rows:
        ...
        if typ == 'solar':
            ...
            base_bias = blended_load_bias(state, typ, model.get('bias'))
            if fc_now > 0.05 and actual >= 0.0:
                est_bias = actual / max(fc_now, 1e-6)
                model['bias'] = 0.88 * safe_float(model.get('bias', base_bias), base_bias) + 0.12 * clamp(est_bias, 0.35, 1.15)
            else:
                model['bias'] = 0.97 * safe_float(model.get('bias', base_bias), base_bias) + 0.03 * base_bias
            pred = blended_load_bias(state, typ, model.get('bias')) * max(fc_now, 0.0)
...
def predict_object_load(state: Dict[str, Any], row: Dict[str, Any], forecast_value: float) -> float:
    ...
    if typ not in OBJECT_TYPE_TO_FORECAST:
        return 0.0
    model = get_model(state, key, 'load')
    bias = blended_load_bias(state, typ, model.get('bias'))
...
def compute_target_soc(cfg: Dict[str, Any], total_capacity: float, total_soc: float, future: List[Dict[str, Any]], fill_ratio: float, tick: int, game_length: int, loss_ratio: float = 0.10) -> float:
    ...
    chronic_deficit = raw_gap_sum > max(4.0, 1.7 * raw_surplus_sum)
    floor = total_capacity * (0.03 if chronic_deficit else SOC_FLOOR_FRAC)
    target = floor + 0.50 * weighted_gap + 0.10 * useful_gap_sum - 0.35 * weighted_surplus
    ...
    if ticks_left <= ENDGAME_TICKS:
        floor = 0.0
        target = 0.10 * weighted_gap
    return clamp(target, floor, base_ceil)


def storage_policy(cfg: Dict[str, Any], storages: List[Dict[str, Any]], balance_now: float, useful_now: float, future: List[Dict[str, Any]], fill_ratio: float, tick: int, game_length: int, loss_ratio: float = 0.10) -> Tuple[List[Tuple[str, float]], List[Tuple[str, float]], Dict[str, Any]]:
    if not storages:
        return [], [], {
            'target_soc': 0.0,
            'total_soc': 0.0,
    ...
    target_soc = compute_target_soc(cfg, total_capacity, total_soc, future, fill_ratio, tick, game_length, loss_ratio=loss_ratio)
    ...
    chronic_deficit = deficit_sum > max(4.0, 1.7 * surplus_sum)
    ...
    protected_soc = max(floor_soc, 0.45 * target_soc if chronic_deficit else 0.72 * target_soc)
    soc_band = max(6.0, 0.08 * total_capacity)
    ...
    need_charge = total_soc < target_soc - soc_band
    need_discharge = total_soc > target_soc + soc_band

    discharge_budget = max(0.0, total_soc - protected_soc)
    if current_deficit > 1.5 and discharge_budget > 0.0 and (need_discharge or next_deficit > 2.0 or chronic_deficit):
        desired = 0.82 * current_deficit + 0.12 * next_deficit
        if chronic_deficit:
            desired += 0.05 * deficit_sum
    ...
    if current_surplus > 2.5 and charge_room > 0.0 and need_charge:
        desired_charge = current_surplus
        desired_charge -= 0.30 * next_deficit
        if chronic_deficit:
            desired_charge *= 0.85
        if next_deficit > current_surplus * 0.9:
            desired_charge *= 0.55
        charge_total = min(total_charge_rate, charge_room, max(0.0, desired_charge))
    elif current_surplus <= 0.0 and next_surplus > 5.0 and charge_room > 0.0 and total_soc < target_soc - soc_band and next_deficit < 0.55 * next_surplus:
        charge_total = min(total_charge_rate, charge_room, 0.35 * next_surplus)
    ...
    if tick >= game_length - ENDGAME_TICKS and total_soc > floor_soc:
        ...
        if balance_now >= -1.0 or useful_now > 0.0:
            discharge_for_market = extra
...
def compute_reserve(state: Dict[str, Any], future: List[Dict[str, Any]], object_rows: List[Dict[str, Any]], network_losses: float, useful_supply_now: float) -> float:
    ...
    reserve = 0.18 + 0.30 * abs_err + 0.08 * max(0.0, loss_ratio - 0.10) * max(gen_total, useful_supply_now)
    reserve += 0.06 * wind_share * max(wind_gen, next_useful)
    if next_useful < useful_supply_now:
        reserve += 0.18 * (useful_supply_now - next_useful)
    if next_balance < -2.0:
        reserve += 0.05 * abs(next_balance)
    if safe_float(state.get('fill_ratio_ewma', 0.74), 0.74) > 0.92 and safe_float(state.get('instant_abs_last', 0.0), 0.0) <= 1e-9:
        reserve *= 0.92
...
def build_ladder(sell_volume: float, market_ref: float, fill_ratio: float, max_tickets: int, cfg: Dict[str, Any], buy_ref: Optional[float] = None, gap_ewma: float = 0.0) -> List[Tuple[float, float]]:
    ...
    base_ref = clamp(max(market_ref, buy_ref), gp_price + step, market_cap - step)

    # Last game showed systematic underpricing: all sell orders filled, while contracted price
    # stayed noticeably above asked price. Use EWMA gap to lift the ladder.
    lift = clamp(0.55 * safe_float(gap_ewma, 0.0), 0.0, 1.8)
    entry = clamp(base_ref - (0.9 if fill_ratio < 0.70 else 0.40), gp_price + step, market_cap - step)
    mid = clamp(base_ref + 0.20 * lift + (0.0 if fill_ratio < 0.70 else 0.20), gp_price + step, market_cap - step)
    peak = clamp(base_ref + 0.65 * lift + 0.45, gp_price + step, market_cap - step)
    high = clamp(base_ref + 1.10 * lift + 0.85, gp_price + step, market_cap - step)

    if sell_volume < 1.5:
        prices = [entry, mid]
        shares = [0.65, 0.35]
    elif fill_ratio < 0.65:
        prices = [entry, mid, peak]
        shares = [0.52, 0.30, 0.18]
    elif fill_ratio > 0.94:
        prices = [entry, mid, peak, high]
        shares = [0.35, 0.35, 0.20, 0.10]
    else:
        prices = [entry, mid, peak]
...
def controller(psm: Any) -> Dict[str, Any]:
    ...
    sell_gap_now = None
    if sell_avg_price is not None and market_stats.get('sell_avg_asked_price') is not None and safe_float(market_stats.get('sell_contracted', 0.0), 0.0) > 0.0:
        sell_gap_now = max(0.0, safe_float(sell_avg_price, 0.0) - safe_float(market_stats.get('sell_avg_asked_price', 0.0), 0.0))
        state['sell_gap_ewma'] = 0.82 * safe_float(state.get('sell_gap_ewma', 1.2), 1.2) + 0.18 * clamp(sell_gap_now, 0.0, 4.0)
    else:
        state['sell_gap_ewma'] = 0.97 * safe_float(state.get('sell_gap_ewma', 1.2), 1.2) + 0.03 * 1.0
    state['instant_abs_last'] = safe_float(market_stats.get('instant_abs_total', 0.0), 0.0)
    ...
    marketable_useful_now = max(0.0, useful_now + battery_dbg['discharge_total'] - battery_dbg['charge_total'])
    physical_sell_cap = marketable_useful_now
    if balance_now < 0.0:
        # In deficit regimes, sell only the portion that remains after the battery first covers
        # internal balance. This prevents aggressive export while the system is still short.
        physical_sell_cap = min(
            physical_sell_cap,
            max(0.0, balance_now + battery_dbg['discharge_total'] - battery_dbg['charge_total']),
        )
    offer_cap = compute_offer_cap(state, cfg, tick, marketable_useful_now)
    reserve = compute_reserve(state, future, object_rows, total_losses, physical_sell_cap)
    sell_target = max(0.0, min(offer_cap, physical_sell_cap) - reserve)
    if physical_sell_cap >= MIN_ORDER_VOLUME and sell_target < MIN_ORDER_VOLUME and balance_now >= -0.5:
        sell_target = min(offer_cap, physical_sell_cap, max(MIN_ORDER_VOLUME, 0.60 * physical_sell_cap))
    sell_volume = round_vol(clamp(sell_target, 0.0, min(offer_cap, physical_sell_cap)))
    if sell_volume < MIN_ORDER_VOLUME:
        sell_volume = 0.0
    ...
    ladder = build_ladder(
        ...
        cfg,
        buy_ref=buy_ref,
        gap_ewma=safe_float(state.get('sell_gap_ewma', 1.2), 1.2),
    )
    ...
    summary_row = {
        ...
        'sell_gap_ewma': round(safe_float(state.get('sell_gap_ewma', 1.2), 1.2), 6),
