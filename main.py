...
def round_price(x: float) -> float:
    return round(round(clamp(float(x), 2.0, 20.0) / 0.2) * 0.2, 2)
...
def obj_wind_rotation_now(obj: Any) -> Optional[float]:
    if is_compact_object(obj):
        ...
        return None
    return safe_float(getattr(obj, 'windRotation', 0.0), 0.0)
...
def get_model(state: Dict[str, Any], key: str, kind: str) -> Dict[str, Any]:
    ...
    if key not in models:
        if kind == 'solar':
            ...
            models[key] = {'kind': kind, 'factor': 0.12, 'err': 1.2}
        else:
    ...
    for row in object_rows:
        ...
        if typ == 'solar':
            ...
            if wind_now > 0.2 and actual >= 0.0:
                ...
                model['factor'] = 0.84 * model['factor'] + 0.16 * clamp(est, 0.0, 5.0)
            pred = model['factor'] * (wind_now ** 3)
            model['err'] = 0.86 * model['err'] + 0.14 * abs(actual - pred)
...
def predict_object_generation(
    ...
    if typ == 'wind':
        ...
        return clamp(model['factor'] * (safe_wind ** 3), 0.0, 20.0)
...
def forecast_window(
    ...
    for t in range(tick, min(last_tick, tick + horizon - 1) + 1):
        ...
        for row in object_rows:
            gen_pred = predict_object_generation(state, row, fc_sun, fc_wind, sun_spread, wind_spread)
...
def current_theoretical_metrics(
    ...
    for row in object_rows:
        typ = row['type']
        gen_theoretical = predict_object_generation(state, row, sun_now, wind_now, sun_spread, wind_spread)
        load_fc = 0.0
        load_model = 0.0
...
def build_ladder(sell_volume: float, market_ref: float, fill_ratio: float, max_tickets: int) -> List[Tuple[float, float]]:
    if sell_volume < MIN_ORDER_VOLUME:
        return []

    market_ref = clamp(market_ref, 3.5, 9.5)
    ...
    prices = [round_price(p) for p in prices]
    out = []
    allocated = 0.0
...
def controller(psm: Any) -> Dict[str, Any]:
    ...
    update_models(state, object_rows, weather, forecast_bundle, tick)
    ...
    current_theoretical, object_prediction_rows = current_theoretical_metrics(
        state, object_rows, weather, forecast_bundle, tick
    )
