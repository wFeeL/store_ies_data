...
tick_log_line = (
    f"TICK={psm.tick} "
    f"SOLD_ENERGY={sell_contracted:.3f} "
    f"SOLD_PRICE={'NA' if sell_avg_contracted_price is None else f'{sell_avg_contracted_price:.3f}'} "
    f"DISCHARGED={ordered_discharged_total:.3f} "
    f"TOTAL_STORAGE_NOW={total_storage_charge:.3f} "
    f"EXCHANGE_BID={ladder_str}"
)
print(tick_log_line)
