...
def compute_reserve(
    ...
    reserve = max(
        0.30,
        0.28 * safe_float(state.get('abs_err_ewma', 1.5), 1.5),
        0.05 * network_losses,
        0.015 * max(0.0, exportable_now),
    )
    if wind_share > 0.35:
        reserve += 0.02 * wind_gen
    if future:
        next_gap = max(0.0, -safe_float(future[0].get('useful_pred', 0.0), 0.0))
        reserve += 0.012 * next_gap
    if exportable_now > 0.0:
        reserve = min(reserve, max(0.35, 0.55 * exportable_now))
    return reserve
...
def controller(psm: Any) -> Dict[str, Any]:
    ...
    if exportable_now >= MIN_ORDER_VOLUME and sell_volume < MIN_ORDER_VOLUME:
        starter = min(offer_cap, exportable_now * 0.35)
        if fill_ewma < 0.55:
            starter = min(offer_cap, exportable_now * 0.50)
