...
def get_model(state: Dict[str, Any], key: str, kind: str) -> Dict[str, Any]:
    ...
    if key not in models:
        if kind == 'solar':
            ...
            models[key] = {'kind': kind, 'factor': 0.12, 'rot_factor': 0.12, 'rot_scale': 1.0, 'err': 1.2, 'last_failed': 0}
        else:
...
def project_wind_rotation(current_rotation: float, current_wind: float, fc_wind: float, delay: int) -> float:
    if current_rotation <= 0.0:
        return max(0.0, fc_wind)
    if current_wind <= 0.1:
        return max(0.0, fc_wind)
    inertia = clamp(0.72 + 0.06 * max(0, min(delay, 3)), 0.55, 0.90)
    target = max(0.0, fc_wind)
    return max(0.0, inertia * current_rotation + (1.0 - inertia) * target)
...
def update_models(
    ...
    for row in object_rows:
        ...
        if typ == 'solar':
            ...
            if sun_now > 0.1 and actual >= 0.0:
                est = actual / max(sun_now, 1e-6)
                model['factor'] = 0.82 * model['factor'] + 0.18 * clamp(est, 0.0, 20.0)
            pred = model['factor'] * sun_now
            model['err'] = 0.85 * model['err'] + 0.15 * abs(actual - pred)

        elif typ == 'wind':
            model = get_model(state, key, 'wind')
            actual = row['generated']
            rotation_now = safe_float(row.get('wind_rotation', 0.0), 0.0)
            if wind_now > 0.2 and rotation_now > 0.1:
                rs = rotation_now / max(wind_now, 1e-6)
                model['rot_scale'] = 0.88 * safe_float(model.get('rot_scale', 1.0), 1.0) + 0.12 * clamp(rs, 0.4, 1.6)
            if rotation_now > 0.1 and actual >= 0.0:
                est_rot = actual / max(rotation_now ** 3, 1e-6)
                model['rot_factor'] = 0.84 * safe_float(model.get('rot_factor', model.get('factor', 0.12)), 0.12) + 0.16 * clamp(est_rot, 0.0, 5.0)
            elif wind_now > 0.2 and actual >= 0.0:
                est = actual / max(wind_now ** 3, 1e-6)
                model['factor'] = 0.84 * safe_float(model.get('factor', 0.12), 0.12) + 0.16 * clamp(est, 0.0, 5.0)
            pred_rot = safe_float(model.get('rot_factor', model.get('factor', 0.12)), 0.12) * (rotation_now ** 3)
            pred_wind = safe_float(model.get('factor', 0.12), 0.12) * (wind_now ** 3)
            pred = pred_rot if rotation_now > 0.1 else pred_wind
            storm_delta = max(0, safe_int(row.get('failed', 0), 0) - safe_int(model.get('last_failed', 0), 0))
            if storm_delta > 0:
                model['err'] = 0.75 * safe_float(model.get('err', 1.2), 1.2) + 0.25 * max(abs(actual - pred), 2.0)
            else:
                model['err'] = 0.86 * safe_float(model.get('err', 1.2), 1.2) + 0.14 * abs(actual - pred)
            model['last_failed'] = safe_int(row.get('failed', 0), 0)
            ...
            if fc_now > 0.05 and actual >= 0.0:
                ...
                model['bias'] = 0.88 * model['bias'] + 0.12 * clamp(est_bias, 0.6, 1.6)
            pred = model['bias'] * max(fc_now, 0.0)
            model['err'] = 0.88 * model['err'] + 0.12 * abs(actual - pred)
...
def predict_object_generation(
    ...
    if typ == 'solar':
        ...
        return clamp(model['factor'] * safe_sun, 0.0, 20.0)
    ...
    if typ == 'wind':
        ...
        safe_wind = max(0.0, fc_wind - 0.45 * wind_spread)
        safe_rot = project_wind_rotation(current_rot, current_wind, safe_wind, delay)
        rot_factor = safe_float(model.get('rot_factor', model.get('factor', 0.12)), 0.12)
        wind_factor = safe_float(model.get('factor', 0.12), 0.12)
        pred_rot = rot_factor * (safe_rot ** 3)
        pred_wind = wind_factor * (safe_wind ** 3)
        pred = 0.65 * pred_rot + 0.35 * pred_wind if current_rot > 0.1 else pred_wind
        if safe_int(row.get('failed', 0), 0) > 0 and current_rot > current_wind * 1.15:
            pred *= 0.85
...
def compute_target_soc(
    ...
    floor = total_capacity * SOC_FLOOR_FRAC
    ceil = total_capacity * SOC_CEIL_FRAC
    weighted_gap = 0.0
    weighted_surplus = 0.0
    for i, row in enumerate(future):
        w = 1.0 / (i + 1)
        gap = row['total_load_pred'] - row['total_gen_pred']
        if gap > 0:
            weighted_gap += w * gap
        else:
            weighted_surplus += w * abs(gap)

    target = floor + 0.48 * weighted_gap - 0.12 * weighted_surplus
    if fill_ratio < 0.65:
        target += 0.10 * total_capacity
    elif fill_ratio > 0.92:
        target -= 0.05 * total_capacity

    ticks_left = game_length - tick
    if ticks_left <= ENDGAME_TICKS:
        target = floor + 0.20 * weighted_gap

    return clamp(target, floor, ceil)
...
def storage_policy(
    ...
    target_soc = compute_target_soc(cfg, total_capacity, total_soc, future, fill_ratio, tick, game_length)
    floor_soc = total_capacity * SOC_FLOOR_FRAC
    charge_total = 0.0
    discharge_total = 0.0
    ...
    # Balanced-conservative battery logic:
    # 1) offset current deficit/import;
    # 2) absorb current surplus if battery below target;
    # 3) use battery for selling only near endgame or if SOC is clearly excessive.
    protected_soc = max(floor_soc, 0.95 * target_soc)
    ...
    if current_deficit > 0.0 and discharge_budget > 0.0:
        discharge_total = min(
            ...
            discharge_budget,
            0.60 * current_deficit + 0.20 * max(0.0, next_gap),
        )
    elif current_surplus > 0.0 and total_soc < target_soc:
        charge_total = min(
            total_charge_rate,
            target_soc - total_soc,
            0.80 * current_surplus,
        )

    if tick >= game_length - ENDGAME_TICKS and total_soc > floor_soc:
        discharge_for_sell = min(
            total_discharge_rate - discharge_total,
            max(0.0, total_soc - floor_soc),
            0.20 * total_soc,
        )
    ...
    for s in sorted(storages, key=lambda x: -x['soc']):
        if rem <= 1e-9:
            break
        avail = max(0.0, s['soc'] - floor_soc / max(1, len(storages)))
        amt = min(rem, discharge_rate, max(0.0, avail))
        if amt >= 1e-9:
