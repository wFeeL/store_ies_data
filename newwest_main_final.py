...
def reset_runtime_outputs() -> None:
    ...
    for path in [
        ...
        FORECAST_ERRORS_CSV,
        DERIVED_METRICS_CSV,
        os.path.join(LOG_DIR, 'object_predictions.csv'),
    ]:
        try:
...
def find_external_forecast_file() -> Optional[str]:
    ...
    if not env_path:
        return None
    full = os.path.abspath(os.path.expanduser(env_path))
...
def log_tick_data(
    ...
    if derived_row is not None:
        append_csv(DERIVED_METRICS_CSV, [
            ...
            'useful_energy_now', 'balance_now', 'exportable_now', 'losses_now',
            ...
            'offer_cap', 'reserve', 'sell_volume', 'topology_warning_count',
    ...
    for row in object_rows:
        ...
        if typ == 'solar':
            ...
            if pred_curve is not None:
                pred = 0.35 * pred_direct + 0.20 * pred_rot + 0.45 * pred_curve
            else:
...
def predict_object_generation(
    ...
    if typ == 'wind':
        ...
        curve_based = estimate_wind_from_curve(model, proj_rot)

        if curve_based is not None:
            pred = 0.28 * direct + 0.22 * rot_based + 0.50 * curve_based
        else:
            pred = 0.58 * direct + 0.42 * rot_based

        max_seen = safe_float(model.get('max_power_seen', 0.0), 0.0)
        if max_seen > 0.0:
            pred = min(pred, min(max_wind_power, 1.15 * max_seen + 0.5))
...
def forecast_window(
    ...
    for t in range(start_tick, end_tick + 1):
        ...
        useful_supply_pred = total_gen - loss_pred
        out.append({
            'tick': t,
            'sun': fc_sun,
            'wind': fc_wind,
...
def compute_useful_energy(total_generated: float, total_consumed: float, total_losses: float) -> float:
    # По правилам полезная энергия — это генерация после потерь.
    # Потребление собственных объектов влияет на баланс энергосистемы,
    # но не уменьшает сам факт полезной энергии, которую система поставила
    # на свои границы и которую нужно продавать через биржу.
...
def compute_balance_energy(total_generated: float, total_consumed: float, total_losses: float) -> float:
    # Физический баланс энергосистемы: если отрицательный, недостача закрывается внешней сетью.
...
def compute_target_soc(
    ...
    for i, row in enumerate(future):
        w = 1.0 / (i + 1)
        gap = max(0.0, -safe_float(row.get('balance_pred', 0.0), 0.0))
        surplus = max(0.0, safe_float(row.get('balance_pred', 0.0), 0.0))
        weighted_gap += w * gap
    ...
    target = floor + 0.40 * weighted_gap - 0.45 * weighted_surplus
    ...
    if ticks_left <= ENDGAME_TICKS:
        ...
        target = floor + 0.12 * weighted_gap
...
def storage_policy(
    ...
    two_step_gap = max(0.0, -safe_float(future[1].get('balance_pred', 0.0), 0.0)) if len(future) > 1 else 0.0
    ...
    protected_soc = floor_soc if chronic_deficit else max(floor_soc, 0.62 * target_soc)
    if next_surplus > 4.0 and next_deficit < 2.0:
        protected_soc = max(floor_soc, 0.48 * target_soc)
    discharge_budget = max(0.0, total_soc - protected_soc)

    if current_deficit > 0.0 and discharge_budget > 0.0:
        desired = current_deficit + 0.12 * next_deficit + 0.05 * two_step_gap
        if chronic_deficit:
            desired += 0.12 * next_deficit
    ...
    if current_surplus > 0.0 and total_soc < total_capacity * SOC_CEIL_FRAC:
        ...
        if chronic_deficit or total_soc < target_soc + 0.10 * total_capacity or next_deficit > 3.0:
            charge_total = min(total_charge_rate, charge_room, desired_charge)

    if current_surplus <= 0.0 and next_surplus > 4.0 and total_soc < target_soc:
        desired_charge = min(total_charge_rate, 0.55 * next_surplus)
        charge_room = total_capacity * SOC_CEIL_FRAC - total_soc
        charge_total = max(charge_total, min(total_charge_rate, charge_room, desired_charge))

    if tick >= game_length - ENDGAME_TICKS and total_soc > floor_soc and balance_now >= -0.5:
...
def compute_physical_sell_cap(
    useful_supply_now: float,
    balance_now: float,
    charge_total: float,
    discharge_total: float,
) -> float:
    """
    Conservative physical cap for sells.

    useful_supply_now reflects generation after losses. balance_now reflects the
    net system balance after own consumption and losses. In deficit regimes the
    controller must not aggressively sell just because it can discharge a battery.
    """
    useful_cap = max(0.0, useful_supply_now + discharge_total - charge_total)
    if useful_cap <= 0.0:
        return 0.0

    balance_cap = max(0.0, balance_now + discharge_total - charge_total)

    # Strong deficit: allow only a tiny probing sale, otherwise the system starts
    # selling while being physically short and shifts risk to the instant market.
    if balance_cap <= 0.0:
        return min(useful_cap, max(0.0, 0.10 * useful_supply_now))

    # Mild deficit or mixed regime: mostly trust the balance cap, with only a
    # small allowance for the useful-energy market metric.
    if balance_cap < useful_cap:
        blended = balance_cap + 0.08 * (useful_cap - balance_cap)
        return min(useful_cap, max(0.0, blended))
...
def analyze_topology(object_rows: List[Dict[str, Any]], network_rows: List[Dict[str, Any]], total_generated: float) -> Dict[str, Any]:
    ...
    branch_losses_sorted = sorted(
        ({'branch': k, **v} for k, v in by_branch.items()),
    ...
    hospital_load = sum(safe_float(r.get('consumed', 0.0), 0.0) for r in object_rows if r.get('type') == 'hospital')
    if hospital_count > 0 and hospital_load <= 0.0:
        warnings.append('hospital_zero_consumption')

    storage_count = sum(1 for r in object_rows if r.get('type') == 'storage')
    if storage_count == 0:
        warnings.append('no_storage')
...
def compute_reserve(
    ...
    solar_gen = sum(r['generated'] for r in object_rows if r['type'] == 'solar')
    wind_share = wind_gen / max(gen_total, 1e-9) if gen_total > 0 else 0.0

    abs_err = safe_float(state.get('abs_err_ewma', 1.5), 1.5)
    loss_ratio = safe_float(state.get('loss_ratio_ewma', 0.10), 0.10)
    next_gap = max(0.0, -safe_float(future[0].get('balance_pred', 0.0), 0.0)) if future else 0.0
    second_gap = max(0.0, -safe_float(future[1].get('balance_pred', 0.0), 0.0)) if len(future) > 1 else 0.0

    reserve_generation = 0.12 * abs_err + 0.012 * wind_gen + 0.004 * solar_gen
    if wind_share > 0.35:
        reserve_generation += 0.06 * abs_err

    reserve_trade = 0.010 * min(max(0.0, useful_supply_now), next_gap + 0.35 * second_gap)
    reserve_losses = max(0.0, loss_ratio - 0.10) * max(gen_total, useful_supply_now)
    reserve = max(0.25, reserve_generation + reserve_trade + reserve_losses)
    if future:
        first_balance = safe_float(future[0].get('balance_pred', 0.0), 0.0)
        if first_balance > 0.0:
            reserve *= 0.92
        elif first_balance < -2.0:
            reserve *= 1.08
    if useful_supply_now > 0.0:
        reserve = min(reserve, max(0.25, 0.42 * useful_supply_now))
    return reserve
...
def build_ladder(sell_volume: float, market_ref: float, fill_ratio: float, max_tickets: int, cfg: Dict[str, Any]) -> List[Tuple[float, float]]:
    ...
    # Наша цель — реализовать как можно больше полезной энергии дороже GP,
    # поэтому стартуем очень низко и небольшой хвост уводим дороже.
    market_ref = clamp(market_ref, gp_price + 0.6, max(gp_price + 1.0, market_cap - 0.4))

    if sell_volume < 1.5:
        prices = [gp_price + 0.2, gp_price + 0.8]
        shares = [0.80, 0.20]
    elif fill_ratio < 0.65:
        prices = [gp_price + 0.2, gp_price + 0.8, gp_price + 1.6]
        shares = [0.78, 0.17, 0.05]
    else:
        prices = [gp_price + 0.4, min(market_ref, gp_price + 1.2), min(market_ref + 1.0, market_cap - 0.2)]
...
def current_theoretical_metrics(
    ...
    sun_spread = max(get_forecast_spread(bundle, 'sun', 0.0), safe_float((cfg or {}).get('corridorSun', 0.5), 0.5))
    wind_spread = max(get_forecast_spread(bundle, 'wind', 0.0), safe_float((cfg or {}).get('corridorWind', 0.5), 0.5))
...
def controller(psm: Any) -> Dict[str, Any]:
    ...
    charge_orders, discharge_orders, battery_dbg = storage_policy(
        cfg, obj_agg['storages'], balance_now, future,
    ...
    physical_sell_cap = compute_physical_sell_cap(
        exportable_now,
        balance_now,
        battery_dbg['charge_total'],
        battery_dbg['discharge_total'],
    )
    sell_cap_now = min(offer_cap, physical_sell_cap)
    sell_volume = physical_sell_cap - reserve
    sell_volume = clamp(sell_volume, 0.0, sell_cap_now)
    fill_ewma = safe_float(state.get('fill_ratio_ewma', 0.72), 0.72)
    if exportable_now >= MIN_ORDER_VOLUME and sell_volume < MIN_ORDER_VOLUME:
        balance_after_battery = balance_now + battery_dbg['discharge_total'] - battery_dbg['charge_total']
        starter_ratio = 0.45
        if fill_ewma < 0.55:
            starter_ratio = 0.55
        if balance_after_battery < 0.0:
            starter_ratio = min(starter_ratio, 0.20)
        starter = min(sell_cap_now, exportable_now * starter_ratio)
        if starter >= MIN_ORDER_VOLUME:
            sell_volume = starter
    ...
    summary_row = {
        ...
        'net_surplus_now': round(balance_now, 6),
        ...
        'sell_cap_now': round(sell_cap_now, 6),
        ...
        'top_loss_branch': topology.get('branch_losses', [{}])[0] if topology.get('branch_losses') else None,
        'type_totals': {
            typ: {
                'count': data['count'],
