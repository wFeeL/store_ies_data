...
def compute_reserve(
    ...
    if useful_supply_now > 0.0:
        reserve = min(reserve, max(0.25, 0.45 * useful_supply_now))
    return reserve
...
def controller(psm: Any) -> Dict[str, Any]:
    ...
    physical_sell_cap = max(0.0, exportable_now + battery_dbg['discharge_total'] - battery_dbg['charge_total'])
    ...
    if exportable_now >= MIN_ORDER_VOLUME and sell_volume < MIN_ORDER_VOLUME:
        starter = min(sell_cap_now, exportable_now * 0.45)
        if fill_ewma < 0.55:
            starter = min(sell_cap_now, exportable_now * 0.60)
