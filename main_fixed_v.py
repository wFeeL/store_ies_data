...
def default_state() -> Dict[str, Any]:
    return {
        ...
        'execution_ratio_ewma': 0.84,
        'market_ref': 4.8,
        'exchange_ref_ewma': 4.8,
        'market_history': [],
...
def build_market_context(state: Dict[str, Any], exch_log: Optional[List[float]] = None, cfg: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    history = list(state.get('market_history', []))
    exch_log = list(exch_log or [])
    cfg = cfg or {}
    step = safe_float(cfg.get('exchangeConsumerPriceStep', 0.2), 0.2)

    fill_samples = [
        safe_float(row.get('fill_ratio', 0.0), 0.0)
        for row in history
        if safe_float(row.get('sell_asked', 0.0), 0.0) > 0.25
    ]
    ask_samples = [
        safe_float(row.get('avg_ask_price', 0.0), 0.0)
        for row in history
        if safe_float(row.get('sell_asked', 0.0), 0.0) > 0.25
    ]
    execution_samples = [
    ...
    weak_fill_ratio = safe_float(MARKET_INSIGHTS.get('weak_fill_ratio', 0.72), 0.72)
    ...
    overpriced_gap_steps = safe_float(MARKET_INSIGHTS.get('overpriced_gap_steps', 1.1), 1.1)
    underpriced_gap_steps = safe_float(MARKET_INSIGHTS.get('underpriced_gap_steps', 1.6), 1.6)
    overpriced_ask = safe_float(MARKET_INSIGHTS.get('overpriced_ask', 9.8), 9.8)

    recent_fill = avg(fill_samples, default=safe_float(state.get('fill_ratio_ewma', 0.84), 0.84))
    recent_ask = avg(ask_samples, default=safe_float(state.get('market_ref', 4.8), 4.8))
    recent_execution = avg(
        execution_samples,
        default=safe_float(state.get('execution_ratio_ewma', recent_fill), recent_fill),
    )

    log_tail = [safe_float(x, None) for x in exch_log[-8:]]
    log_tail = [x for x in log_tail if x is not None and x > 0.0]
    exchange_market_ref = avg(log_tail, default=safe_float(state.get('exchange_ref_ewma', state.get('market_ref', 4.8)), 4.8))
    internal_market_ref = safe_float(state.get('market_ref', exchange_market_ref), exchange_market_ref)
    market_ref = 0.70 * exchange_market_ref + 0.30 * internal_market_ref

    step = max(0.01, step)
    ask_gap_steps = (recent_ask - market_ref) / step
    has_fill_history = len(fill_samples) >= 1
    has_execution_history = len(execution_samples) >= 1
    near_zero_fill = (
        (has_fill_history and recent_fill <= near_zero_fill_ratio)
        or (has_execution_history and recent_execution <= 0.5 * poor_execution_ratio)
    )

    weak_market = recent_fill < weak_fill_ratio or recent_execution < weak_fill_ratio
    strong_market = recent_fill >= good_fill_ratio and recent_execution >= good_execution_ratio

    overpriced = bool(
        (recent_ask >= max(overpriced_ask, market_ref + 0.5 * step) and weak_market)
        or (ask_gap_steps >= overpriced_gap_steps and (weak_market or near_zero_fill))
    )
    underpriced = bool(
        ask_gap_steps <= -underpriced_gap_steps
        and strong_market
    )
    ...
    if near_zero_fill:
        undercut_steps = 3
    elif recent_fill < bad_fill_ratio or recent_execution < poor_execution_ratio:
        undercut_steps = 2
    elif recent_fill < weak_fill_ratio or recent_execution < weak_fill_ratio:
        undercut_steps = 1
    elif underpriced:
        undercut_steps = -1

    target_sell_price = market_ref - undercut_steps * step
    if overpriced:
        target_sell_price = min(target_sell_price, market_ref - step)
    if underpriced:
        target_sell_price = max(target_sell_price, market_ref)

    target_sell_price = round_price(target_sell_price, price_step=step)

    if near_zero_fill:
        price_realism = 0.45
    elif overpriced:
        price_realism = 0.55 if undercut_steps >= 2 else 0.68
    elif weak_market:
        fit = min(recent_fill, recent_execution) / max(weak_fill_ratio, 1e-6)
        price_realism = clamp(0.70 + 0.30 * fit, 0.52, 0.96)
    else:
        price_realism = clamp(0.86 + 0.14 * min(recent_fill, recent_execution), 0.72, 1.0)

    adaptive_volume_scale = 1.0
    if near_zero_fill:
        adaptive_volume_scale = 0.32
    elif overpriced and weak_market:
        adaptive_volume_scale = 0.58 + 0.25 * price_realism
    elif weak_market:
        adaptive_volume_scale = 0.68 + 0.32 * price_realism
    elif underpriced and strong_market:
        adaptive_volume_scale = 1.08
    adaptive_volume_scale = clamp(adaptive_volume_scale, 0.30, 1.15)

    if near_zero_fill:
        sell_regime = 'blocked'
    elif overpriced and weak_market:
        sell_regime = 'defensive'
    elif weak_market:
        sell_regime = 'cautious'
    elif underpriced and strong_market:
        sell_regime = 'aggressive'
    else:
        sell_regime = 'normal'

    return {
        'history': history,
        'exchange_market_ref': exchange_market_ref,
        ...
        'good_fill': strong_market,
        'weak_fill': weak_market,
        'overpriced': overpriced,
        'underpriced': underpriced,
        'near_zero_fill': near_zero_fill,
        'ask_gap_steps': ask_gap_steps,
        'adaptive_volume_scale': adaptive_volume_scale,
        'sell_regime': sell_regime,
        'undercut_steps': undercut_steps,
    }
...
def storage_policy(state: Dict[str, Any], cfg: Dict[str, Any], storages: List[Dict[str, Any]], balance_now: float, future: List[Dict[str, Any]], fill_ratio: float, tick: int, game_length: int, loss_ratio: float = 0.10, profile_ctx: Optional[Dict[str, Any]] = None, market_ctx: Optional[Dict[str, Any]] = None) -> Tuple[List[Tuple[str, float]], List[Tuple[str, float]], Dict[str, Any]]:
    if not storages:
        return [], [], {
            ...
            'premium_market_window': False,
            'fill_to_ceiling': False,
            'market_soc_floor': 0.0,
            'weak_market_fill': False,
    ...
    predicted_deficit = max(0.0, -next_balance) + 0.5 * max(0.0, -next2_balance)

    recent_fill = safe_float(market_ctx.get('recent_fill_ratio', fill_ratio), fill_ratio) if market_ctx else fill_ratio
    recent_execution = safe_float(market_ctx.get('recent_execution_ratio', recent_fill), recent_fill) if market_ctx else recent_fill
    weak_fill_ratio = safe_float(MARKET_INSIGHTS.get('weak_fill_ratio', 0.72), 0.72)
    good_fill_ratio = safe_float(MARKET_INSIGHTS.get('good_fill_ratio', 0.88), 0.88)
    good_execution_ratio = safe_float(MARKET_INSIGHTS.get('good_execution_ratio', 0.72), 0.72)
    ...
    weak_market_fill = bool(market_ctx.get('weak_fill', False)) if market_ctx else (recent_fill < weak_fill_ratio or recent_execution < weak_fill_ratio)
    market_overpriced = bool(market_ctx.get('overpriced', False)) if market_ctx else False
    market_underpriced = bool(market_ctx.get('underpriced', False)) if market_ctx else False
    ...
    adaptive_volume_scale = safe_float(market_ctx.get('adaptive_volume_scale', 1.0), 1.0) if market_ctx else 1.0
    ...
    protect_bias = safe_float(current.get('protect_bias', 0.0), 0.0)
    charge_bias = safe_float(current.get('charge_bias', 0.0), 0.0)
    risk_soon = next_risk_in is not None and 0 <= next_risk_in <= 8
    risk_very_soon = next_risk_in is not None and 0 <= next_risk_in <= 4
    mixed_soon = next_mixed_in is not None and 0 <= next_mixed_in <= 6

    emergency_floor_frac = 0.10
    if loss_ratio > 0.24:
        emergency_floor_frac += min(0.04, 0.18 * (loss_ratio - 0.24))
    if near_zero_fill:
        emergency_floor_frac += 0.02
    if risk_very_soon:
        emergency_floor_frac += 0.02
    emergency_floor_soc = clamp(emergency_floor_frac, 0.10, 0.18) * total_capacity

    working_floor_frac = 0.22
    if protect_bias > 0.5 or avg_risk > avg_combo + 0.10:
        working_floor_frac += 0.06
    elif charge_bias > 0.5 and avg_combo >= avg_risk:
        working_floor_frac += 0.02
    if weak_market_fill and (has_fill_history or has_execution_history):
        working_floor_frac += 0.04
    if price_realism < 0.65:
        working_floor_frac += 0.03
    if anti_dump_headroom < max(2.0, 0.06 * total_discharge_rate):
        working_floor_frac += 0.02
    if predicted_deficit > 2.5:
        working_floor_frac += min(0.06, 0.01 * predicted_deficit)
    working_floor_soc = clamp(
        working_floor_frac * total_capacity,
        emergency_floor_soc + 0.08 * total_capacity,
        min(soc_ceil, 0.64 * total_capacity),
    )

    market_soc_floor = working_floor_soc
    if weak_market_fill:
        market_soc_floor += 0.05 * total_capacity
    if market_overpriced:
        market_soc_floor += 0.04 * total_capacity
    if near_zero_fill:
        market_soc_floor += 0.06 * total_capacity
    if price_realism < 0.60:
        market_soc_floor += 0.04 * total_capacity
    if anti_dump_headroom < max(2.0, 0.08 * total_discharge_rate):
        market_soc_floor += 0.03 * total_capacity
    if risk_soon:
        market_soc_floor += 0.05 * total_capacity
    elif next_risk_in is not None and 0 <= next_risk_in <= 16:
        market_soc_floor += 0.02 * total_capacity
    if mixed_soon and risk_soon:
        market_soc_floor += 0.02 * total_capacity
    market_soc_floor = clamp(
        market_soc_floor,
        emergency_floor_soc + 0.10 * total_capacity,
        min(soc_ceil, 0.82 * total_capacity),
    )
    ...
    target_soc = max(target_soc, working_floor_soc)
    if predicted_deficit > 2.0:
        target_soc += min(0.18 * total_capacity, 0.38 * predicted_deficit)
    if risk_soon:
        target_soc = max(target_soc, market_soc_floor + 0.05 * total_capacity)
    elif mixed_soon and not risk_very_soon:
        target_soc = max(target_soc, working_floor_soc + 0.04 * total_capacity)

    endgame = tick >= game_length - ENDGAME_TICKS
    if endgame:
        target_soc = emergency_floor_soc
    floor_target_soc = emergency_floor_soc if endgame else working_floor_soc
    target_soc = clamp(target_soc, floor_target_soc, min(soc_ceil, 0.84 * total_capacity))

    prep_soc = max(target_soc, emergency_floor_soc if endgame else working_floor_soc)
    protected_soc = emergency_floor_soc if endgame else max(working_floor_soc, market_soc_floor)
    high_risk_target_soc = max(target_soc, protected_soc + (0.06 * total_capacity if risk_soon and not endgame else 0.0))
    high_risk_target_soc = clamp(high_risk_target_soc, protected_soc, min(soc_ceil, 0.90 * total_capacity))
    floor_soc = emergency_floor_soc

    current_deficit = max(0.0, -balance_now)
    current_surplus = max(0.0, balance_now)
    signal = 0.60 * balance_now + 0.30 * next_balance + 0.10 * next2_balance

    charge_total = 0.0
    discharge_total = 0.0
    discharge_for_market = 0.0
    mode = 'hold'
    discharge_floor_soc = emergency_floor_soc

    premium_market_window = bool(
        market_underpriced
        and recent_fill >= good_fill_ratio
        and recent_execution >= good_execution_ratio
        and price_realism >= 0.88
        and not weak_market_fill
        and not risk_very_soon
    )
    fill_to_ceiling = bool(
        not endgame
        and total_soc < soc_ceil - 0.03 * total_capacity
        and (current.get('mixed_peak') or charge_bias > 0.5)
        and (next_risk_in is not None and 0 <= next_risk_in <= 20)
        and not market_overpriced
    )
    must_recharge = (
        total_soc < protected_soc
        or (predicted_deficit > 3.0 and total_soc < high_risk_target_soc)
        or fill_to_ceiling
    )

    market_discharge_margin = (0.06 if premium_market_window else 0.10) * total_capacity
    allow_market_discharge = (
        not endgame
        and total_soc > max(prep_soc, market_soc_floor) + market_discharge_margin
        and recent_fill >= good_fill_ratio
        and recent_execution >= good_execution_ratio
        and signal >= -1.0
        and tick < game_length - 1
        and not weak_market_fill
        and not market_overpriced
        and not near_zero_fill
        and not risk_very_soon
        and adaptive_volume_scale >= 0.75
        and anti_dump_headroom >= MIN_ORDER_VOLUME
    )
    premium_sell_ready = allow_market_discharge and premium_market_window and total_soc > soc_ceil - 0.08 * total_capacity
    deficit_relief_floor_soc = emergency_floor_soc
    if risk_soon and not endgame:
        deficit_relief_floor_soc = min(protected_soc, emergency_floor_soc + 0.10 * total_capacity)

    if current_deficit >= MIN_ORDER_VOLUME and total_soc > emergency_floor_soc:
        mode = 'discharge'
        wanted = current_deficit + 0.35 * max(0.0, -next_balance)
        if risk_very_soon and total_soc <= high_risk_target_soc:
            wanted *= 0.92
        discharge_floor_soc = deficit_relief_floor_soc if not endgame else emergency_floor_soc
        discharge_total = min(total_discharge_rate, wanted, max(0.0, total_soc - discharge_floor_soc))
    elif must_recharge and current_surplus >= MIN_ORDER_VOLUME:
        mode = 'charge'
        charge_target_soc = high_risk_target_soc if risk_soon else prep_soc
        if fill_to_ceiling:
            charge_target_soc = max(charge_target_soc, soc_ceil - 0.01 * total_capacity)
        charge_goal = max(0.0, charge_target_soc - total_soc)
        charge_total = min(total_charge_rate, current_surplus, charge_goal, max(0.0, soc_ceil - total_soc))
    elif allow_market_discharge:
        mode = 'discharge'
        market_discharge_floor = max(market_soc_floor, protected_soc)
        extra_room = max(0.0, total_soc - market_discharge_floor)
        discharge_cap = 0.45 * total_discharge_rate
        if premium_market_window:
            discharge_cap = 0.62 * total_discharge_rate
        if signal < 0.0:
            discharge_cap *= 0.85
        if anti_dump_headroom > 0.0:
            discharge_cap = min(discharge_cap, anti_dump_headroom)
        discharge_for_market = min(total_discharge_rate, extra_room, discharge_cap)
        discharge_total = discharge_for_market
        discharge_floor_soc = market_discharge_floor
    ...
    for s in sorted(norm_storages, key=lambda x: -x['soc']):
        if rem <= 1e-9:
            break
        available = max(0.0, s['soc'] - floor_per_cell)
        amt = min(rem, discharge_rate, available)
        if amt >= 1e-9:
            discharge_orders.append((s['id'], round_vol(amt)))
            rem -= amt

    charge_done = sum(v for _, v in charge_orders)
    discharge_done = sum(v for _, v in discharge_orders)
    discharge_for_market = min(discharge_for_market, discharge_done)

    chronic_deficit = bool(
        predicted_deficit > max(2.0, current_surplus + 1.0)
        or (risk_soon and avg_risk > avg_combo + 0.10)
    )
    ...
    return charge_orders, discharge_orders, {
        ...
        'total_soc': total_soc,
        'soc_ceil': soc_ceil,
        'charge_total': charge_done,
        'discharge_total': discharge_done,
        'discharge_for_market': discharge_for_market,
        'chronic_deficit': chronic_deficit,
        'floor_soc': floor_soc,
        'soc_band': max(0.0, target_soc - emergency_floor_soc),
        ...
        'market_soc_floor': market_soc_floor,
        'weak_market_fill': weak_market_fill,
...
def build_ladder(sell_volume: float, market_ref: float, fill_ratio: float, max_tickets: int, cfg: Dict[str, Any], buy_ref: Optional[float] = None, profile_ctx: Optional[Dict[str, Any]] = None, market_ctx: Optional[Dict[str, Any]] = None, startup_mode: bool = False, storage_excess: bool = False) -> List[Tuple[float, float]]:
    ...
    step = safe_float(cfg.get('exchangeConsumerPriceStep', 0.2), 0.2)
    max_tickets = max(0, safe_int(cfg['exchangeMaxTickets'], 0))

    target_price = safe_float(market_ctx.get('target_sell_price', market_ref), market_ref) if market_ctx else market_ref
    recent_fill = safe_float(market_ctx.get('recent_fill_ratio', fill_ratio), fill_ratio) if market_ctx else fill_ratio
    weak_fill = bool(market_ctx.get('weak_fill', False)) if market_ctx else recent_fill < safe_float(MARKET_INSIGHTS.get('weak_fill_ratio', 0.72), 0.72)
    good_fill = bool(market_ctx.get('good_fill', False)) if market_ctx else recent_fill >= safe_float(MARKET_INSIGHTS.get('good_fill_ratio', 0.88), 0.88)
    underpriced = bool(market_ctx.get('underpriced', False)) if market_ctx else False
    exchange_market_ref = safe_float(market_ctx.get('exchange_market_ref', market_ref), market_ref) if market_ctx else market_ref

    base_ref = target_price
    ...
    if sell_volume <= 3.0:
        prices = [base_ref, base_ref + 2.0 * step, base_ref + 4.0 * step]
        shares = [0.50, 0.30, 0.20]
    elif sell_volume <= 10.0:
        prices = [base_ref - 0.5 * step, base_ref + 1.5 * step, base_ref + 3.0 * step]
        shares = [0.68, 0.22, 0.10]
    elif sell_volume <= 20.0:
        prices = [base_ref - 1.0 * step, base_ref + 1.0 * step, base_ref + 2.0 * step]
        shares = [0.78, 0.17, 0.05]
    else:
        prices = [base_ref - 1.5 * step, base_ref, base_ref + 1.5 * step]
        shares = [0.84, 0.12, 0.04]

    if startup_mode:
        prices = [base_ref - step, base_ref + step]
        shares = [0.85, 0.15]

    if weak_fill:
        prices[0] = min(prices[0], exchange_market_ref - step)

    # Большой объём должен уходить по средней рыночной цене или немного ниже.
    if sell_volume > 10.0:
        prices[0] = min(prices[0], exchange_market_ref)
    if sell_volume > 20.0:
        prices[0] = min(prices[0], exchange_market_ref - 0.5 * step)

    prices = [round_price(max(gp_price + step, p), price_max=market_cap, price_step=step) for p in prices]
...
def compute_safe_sell_volume(state: Dict[str, Any], object_rows: List[Dict[str, Any]], marketable_useful_now: float, offer_cap: float, reserve: float, balance_now: float, topology: Dict[str, Any], market_ctx: Dict[str, Any]) -> float:
    ...
    recent_execution = safe_float(market_ctx.get('recent_execution_ratio', safe_float(state.get('execution_ratio_ewma', 0.84), 0.84)), 0.84)
    weak_fill_ratio = safe_float(MARKET_INSIGHTS.get('weak_fill_ratio', 0.72), 0.72)
    poor_execution_ratio = safe_float(MARKET_INSIGHTS.get('poor_execution_ratio', 0.35), 0.35)
    good_execution_ratio = safe_float(MARKET_INSIGHTS.get('good_execution_ratio', 0.72), 0.72)
    price_realism = safe_float(market_ctx.get('price_realism', 1.0), 1.0)
    ...
    ask_gap_steps = safe_float(market_ctx.get('ask_gap_steps', 0.0), 0.0)
    adaptive_volume_scale = safe_float(market_ctx.get('adaptive_volume_scale', 1.0), 1.0)
    anti_dump_headroom = safe_float(market_ctx.get('anti_dump_headroom', offer_cap), offer_cap)

    warnings = topology.get('warnings', []) if isinstance(topology, dict) else []
    loss_share_est = safe_float(topology.get('loss_share_est', 0.0), 0.0) if isinstance(topology, dict) else 0.0
    high_network_losses = ('high_network_losses' in warnings) or loss_share_est > 0.26

    uncertainty = 0.18 * abs_err + 0.08 * wind_share * marketable_useful_now
    uncertainty += 0.08 * max(0.0, loss_ratio - 0.12) * marketable_useful_now
    uncertainty += 0.10 * max(0.0, 1.0 - price_realism) * marketable_useful_now
    if has_fill_history and recent_fill < weak_fill_ratio:
        uncertainty += 0.05 * (weak_fill_ratio - recent_fill) / max(weak_fill_ratio, 1e-6) * marketable_useful_now
    if has_execution_history and recent_execution < poor_execution_ratio:
        uncertainty += 0.08 * (poor_execution_ratio - recent_execution + 0.15) * marketable_useful_now
    if high_network_losses:
        uncertainty += 0.08 * max(0.0, loss_share_est - 0.22) * marketable_useful_now + 0.4

    safe_target = max(0.0, min(offer_cap, marketable_useful_now) - max(reserve, uncertainty))

    if near_zero_fill:
        safe_target *= 0.34
    elif has_execution_history and recent_execution < poor_execution_ratio:
        safe_target *= 0.70
    elif has_fill_history and recent_fill < weak_fill_ratio:
        safe_target *= 0.86
    elif market_ctx.get('good_fill'):
        safe_target *= 1.04

    if overpriced:
        safe_target *= clamp(0.58 + 0.32 * price_realism, 0.45, 0.88)
    elif ask_gap_steps > 0.8:
        safe_target *= clamp(1.0 - 0.10 * min(ask_gap_steps, 3.0), 0.62, 1.0)

    if underpriced and recent_fill >= weak_fill_ratio and recent_execution >= good_execution_ratio:
        safe_target *= 1.08
    if ask_gap_steps < -1.0 and recent_execution >= good_execution_ratio:
        safe_target *= 1.04

    safe_target *= clamp(adaptive_volume_scale, 0.25, 1.20)
    if high_network_losses:
        safe_target *= 0.82

    if anti_dump_headroom >= 0.0:
        safe_target = min(safe_target, anti_dump_headroom)
...
def controller(psm: Any) -> Dict[str, Any]:
    ...
    sell_contracted_now = safe_float(market_stats.get('sell_contracted', 0.0), 0.0)
    ...
    log_tail = [x for x in log_tail if x is not None and x > 0.0]
    if log_tail:
        exchange_ref = avg(log_tail, default=safe_float(state.get('exchange_ref_ewma', 4.8), 4.8))
        state['exchange_ref_ewma'] = 0.72 * safe_float(state.get('exchange_ref_ewma', 4.8), 4.8) + 0.28 * exchange_ref
        state['market_ref'] = 0.70 * safe_float(state.get('market_ref', 4.8), 4.8) + 0.30 * safe_float(state.get('exchange_ref_ewma', 4.8), 4.8)
    ...
    if prev_sell_order >= MIN_ORDER_VOLUME:
        execution_ratio_now = clamp(sell_contracted_now / max(prev_sell_order, 1e-6), 0.0, 1.5)
        state['execution_ratio_ewma'] = 0.76 * safe_float(state.get('execution_ratio_ewma', 0.84), 0.84) + 0.24 * execution_ratio_now
    ...
    if sell_avg_price is not None and fill_ratio_now is not None and fill_ratio_now >= safe_float(MARKET_INSIGHTS.get('weak_fill_ratio', 0.72), 0.72):
        state['market_ref'] = 0.84 * safe_float(state.get('market_ref', 4.8), 4.8) + 0.16 * sell_avg_price

    update_market_history(state, tick, market_stats, execution_ratio=execution_ratio_now)
    market_ctx = build_market_context(state, exch_log=exch_log, cfg=cfg)
    state['market_ref'] = safe_float(market_ctx.get('market_ref', state.get('market_ref', 4.8)), safe_float(state.get('market_ref', 4.8), 4.8))
    state['sell_bias_steps'] = safe_float(market_ctx.get('ask_gap_steps', 0.0), 0.0)
    state['sell_vol_scale'] = safe_float(market_ctx.get('adaptive_volume_scale', 1.0), 1.0)
    ...
    if startup_mode and battery_dbg.get('signal', 0.0) <= reserve + 0.5 and battery_dbg['total_soc'] < battery_dbg['working_floor_soc']:
        sell_volume = 0.0
    storage_excess = battery_dbg['total_soc'] > battery_dbg.get('target_soc', 0.0) + 0.10 * len(obj_agg['storages']) * safe_float(cfg['cellCapacity'], 120.0)
    ladder = build_ladder(
        sell_volume,
    ...
    summary_row = {
        ...
        'sell_regime': market_ctx.get('sell_regime', 'normal'),
        'sell_bias_steps': round(safe_float(state.get('sell_bias_steps', 0.0), 0.0), 4),
        'sell_vol_scale': round(safe_float(state.get('sell_vol_scale', 1.0), 1.0), 4),
        'ladder': ladder,
    }
